let Calendar = (function(){
    ...
    let Calendar = function(container){
        ...
        const currentMonth = (target) =>
        {
            ...
            selector(target);
            return newValueInTag(container.lang.months[container.global.currentMonth]);
        }
        ...
        const renderHeader = () =>
        {
            ...
            <div class="calendarHeader">\
                <div>\
                    ...
                    <h1 id="calendarHeaderMonth">'+container.lang.months[container.global.currentMonth]+'</h1>\
                    <span id="calendarHeaderYear">'+container.global.currentYear+'</span>\
        ...
        const renderBody = (callback) =>
        {
            ...
            {
                renderStr += '<span data-date="'+ (totalDaysIntLastMonth - (i-1)) +'-'+ (container.global.currentMonth - 1) +'-'+ container.global.currentYear +'" class="calendarLastAndNextMonth">' + (totalDaysIntLastMonth - (i-1)) + '</span>';
            }
            ...
            Array.prototype.forEach.call(this.bodyToRender[0].childNodes, function(element){
                if(element.className === container.tags.classes.calendarBody.substring(1)){
            ...
            if(appendChild){ this.bodyToRender[0].appendChild(body) }
        ...
        const addCalendarEvent = (eventsObj) =>
        {
            ...
            if(typeof this.eventsSave === 'object' && Array.isArray(this.eventsSave) === true)
            {
            ...
            Array.prototype.forEach.call(this.selected, function(element){
                ...
                while(eventsLength && typeof this.eventsSave.events[eventsLength-1] === 'object' && Array.isArray(this.eventsSave.events[eventsLength-1]) === false)
                {
                    element.childNodes.forEach(function(span){
                        if(span.getAttribute('data-date') == this.eventsSave.events[eventsLength-1].data)
                        {
                            let spanNew = document.createElement('span');
                            spanNew.title = 'Visualizar eventos';
                            spanNew.innerHTML = span.innerText;

                            span.className = container.tags.classes.calendarEvent.substring(1);
                            span.innerHTML = '';
